...
#For LUTI we have five durations (1/2/3/4/5)
...
formula3.4<-function(x) {
  return(0.8*exp(-1*exp(-0.99*(x-0.5))))
}

# Scenario 12: Gompertz model, b=1:
formula4.4<-function(x) {
  return(0.8*exp(-1*exp(-1*(x-1))))
}
