...
#Replication of Table 2 of 2020 Quartagno paper
...
for (s in 1:n.scen) {
  ...
  t<-length(x.dur)-1
  while (t>0) {
    if ((formula.1(x.dur[t])+0.125)>max.p) {
      flag=0
  ...
  #Amended   while ((t<=length(poss.durations))&(flag==1)) { from t< to t<=
  ...
  for (i in 1:n.sim) {
    ...
    find.min.dur<- function (data.mfp, indices) {
      ...
      #Define acceptability curve. Constant at 12.5% less than control.
      acceptability<-function(x) {
        return(y.dur.est[100]-0.125+0*x)
      }
    ...
    #Modified "while ((t<length(poss.durations))&(flag==1)) {" by adding a plus 1
    
    flag=t=1
    min.duration<-max.dur
    while ((t<length(poss.durations)+1)&(flag==1)) {
      if ((up.bounds.CI[t]-0.125)<0) {
        flag=0
